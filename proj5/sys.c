...
static int do_tasktree(struct task_struct * root, struct taskinfo * task_array, int nr, int level)
{
	...
	Left to get - child_pid (if applicable)
		      comm?
	...
	*/
	...
	if (list_empty(&root->children)) {
		//append processs info to list
		//empty list, return before iterating (links back to head)
		return count;
	}

	list_for_each(process_entry, &root->children)
	{
		
		printk("CHILD FOUND HERE");
		root = list_entry(process_entry, struct task_struct, sibling);
		if (count == 1) {
...
SYSCALL_DEFINE3(tasktree, struct taskinfo *, buf, int, nr, int, rootpid)
{
	struct taskinfo * kernel_buf;
	//found documentation that states that this function already checks and ensures that the access is okay.
	struct taskinfo * task_array;
	struct task_struct * root;
	int total;

	copy_from_user(buf, kernel_buf, sizeof(buf));


	//remove next two checks when submitting, just using to ensure test cases are correct.
	if (kernel_buf == NULL) {
		printk("ERROR - kernel_buf causing problems...");
	}

	if (nr < 0) {

		printk("ERROR - nr causing problems...");
	}
	
	if (kernel_buf == NULL || nr < 0)
	{
